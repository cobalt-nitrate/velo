...
function DataPreview({ table, toolId }: { table: TabularPreview; toolId: string }) {
  ...
  return (
    <div className="mt-2 overflow-hidden rounded-lg border border-velo-line bg-black/20">
      <div className="border-b border-velo-line bg-white/5 px-2 py-1.5 text-[10px] font-medium uppercase tracking-wide text-velo-muted">
      ...
      <div className="max-h-48 overflow-auto">
        <table className="w-full min-w-[280px] border-collapse text-left text-[11px]">
          <thead>
            <tr className="border-b border-velo-line/80 bg-black/30">
              {columns.map((c) => (
          ...
          <tbody>
            {show.map((row, ri) => (
              <tr key={ri} className="border-b border-velo-line/40 odd:bg-white/[0.02]">
                {columns.map((c) => (
...
export function AgentLivePanel({
  ...
  return (
    <aside
      className={`flex flex-col border-velo-line bg-velo-panel/90 ${className}`.trim()}
    >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        {running ? (
          <div className="mb-3 flex items-center gap-2 rounded-lg border border-velo-accent/30 bg-velo-accent/10 px-2 py-1.5 text-[11px] text-velo-accent">
            <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-velo-accent" />
            Run in progress…
          </div>
        ) : null}
          ...
          <section className="mb-4">
            ...
            <ul className="mt-1 flex flex-wrap gap-1">
              {agentStack.map((id) => (
                <li
                  key={id}
                  className="rounded-md border border-velo-line bg-black/20 px-2 py-0.5 font-mono text-[10px] text-velo-text"
                >
                  {id}
                </li>
              ))}
        ...
        {liveThought.trim() ? (
          <section className="mb-4 rounded-lg border border-white/10 bg-black/25 p-2">
            <h3 className="text-[10px] font-semibold uppercase text-velo-muted">Model</h3>
            ...
            {events.map((e, i) => {
              if (e.type === 'tool.proposed') {
                return (
                  <li
                    key={`p-${i}`}
                    className="rounded-md border border-velo-line/80 bg-black/15 px-2 py-1.5 text-[11px]"
                  >
              ...
              if (e.type === 'tool.executing') {
                return (
                  <li key={`x-${i}`} className="flex items-center gap-2 text-[11px] text-amber-200/90">
              ...
              if (e.type === 'tool.result') {
                return (
                  <li
                    key={`r-${i}`}
                    className="rounded-md border border-emerald-500/25 bg-emerald-500/5 px-2 py-1.5"
                  >
                    <span className="font-mono text-[11px] text-emerald-300">{e.tool_id}</span>
              ...
              if (e.type === 'sub_agent.start') {
                return (
                  <li key={`ss-${i}`} className="text-[11px] text-cyan-300/90">
              ...
              if (e.type === 'sub_agent.end') {
                return (
                  <li key={`se-${i}`} className="text-[10px] text-velo-muted">
                    ← {e.child_agent}{' '}
