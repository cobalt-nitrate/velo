...
export interface OperationalSnapshot {
  ...
  compliance_upcoming: Record<string, string>[];
...
async function gatherOperationalSnapshot(): Promise<OperationalSnapshot> {
  ...
  let compliance_upcoming: Record<string, string>[] = [];
  ...
  if (comp && comp.ok === false) {
    ...
    compliance_upcoming = (comp.rows as Record<string, string>[]).slice(0, 20);
  ...
  else if (typeof arP?.count === 'number') open_ar_receivables = arP.count;
  ...
  else if (typeof emp?.headcount === 'number') active_employees = emp.headcount;
