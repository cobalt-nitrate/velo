...
function arInvoiceHtml(p: Record<string, unknown>): string {
  const inv = esc(p.invoice_number ?? p.invoice_id ?? '—');
  const client = esc(p.client_name ?? 'Client');
  const idate = esc(p.invoice_date ?? '');
  const due = esc(p.due_date ?? '');
  const sub = Number(p.subtotal ?? 0);
  const igst = Number(p.igst ?? 0);
  const cgst = Number(p.cgst ?? 0);
  const sgst = Number(p.sgst ?? 0);
  const total = Number(p.total_amount ?? sub + igst + cgst + sgst);
  const svc = esc(p.service_description ?? 'Professional services');
  const companyName = esc(p.company_name ?? process.env.VELO_COMPANY_NAME ?? 'Your Company');

  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Invoice ${inv}</title>${STYLE}</head>
  <body>
    <div class="header-row">
      <div><h1>${companyName}</h1><div class="muted">Tax Invoice</div></div>
      <div class="badge">AR</div>
    </div>
    <table>
      <tr><td class="label">Invoice #</td><td class="value">${inv}</td>
          <td class="label">Bill to</td><td>${client}</td></tr>
      <tr><td class="label">Invoice date</td><td>${idate}</td>
          <td class="label">Due date</td><td>${due}</td></tr>
    </table>
    <h2>Details</h2>
    <p>${svc}</p>
    <table>
      <tr><th>Description</th><th class="right">Amount</th></tr>
      <tr><td>Taxable value</td><td class="right">${inr(sub)}</td></tr>
      ${igst > 0 ? `<tr><td>IGST</td><td class="right">${inr(igst)}</td></tr>` : ''}
      ${cgst > 0 || sgst > 0 ? `<tr><td>CGST / SGST</td><td class="right">${inr(cgst + sgst)}</td></tr>` : ''}
      <tr class="net"><td>Total (INR)</td><td class="right">${inr(total)}</td></tr>
    </table>
    <div class="muted" style="margin-top:24px;">Generated by Velo — ${new Date().toLocaleDateString('en-IN')}</div>
  </body></html>`;
}
...
function buildHtml(toolId: string, params: Record<string, unknown>): { html: string; filename: string } {
  if (toolId.includes('generate_invoice') || toolId.includes('pdf_generator')) {
    const num = params.invoice_number ?? params.invoice_id ?? 'inv';
    return {
      html: arInvoiceHtml(params),
      filename: `invoice_${String(num).replace(/\s+/g, '_')}.html`,
    };
  }
